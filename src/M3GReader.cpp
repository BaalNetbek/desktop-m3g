...
M3GReader:: M3GReader (const char* name) : stream(0), file_name(name), 
                                           object_index(1), start_of_section(0), size_of_object(0),
...
bool M3GReader:: startSection ()
{
    ...
    if (compression_scheme ==  1) {
        unsigned long compressed_objects_length = total_section_length - 13;
...
{
    // read checksum, but do nothing.
    // because it was already tested.
    getUInt32();
...
void M3GReader:: endObject  ()
{
    ...
    if (end_of_object - (start_of_object + 5) != size_of_object) {
        cout << "M3GReader: object size error.\n";
        cout << "  Perhaps, illegal usage of M3GReader.\n";
        cout << "  file's written size  = " << size_of_object  << "\n";
...
unsigned char* M3GReader:: getUByteArray (int size)
{
    ...
    stream->read ((char*)ret, size*sizeof(unsigned char));
    return ret;
}
...
unsigned short*          M3GReader:: getUInt16Array (int size)
{
    ...
    stream->read ((char*)ret, size*sizeof(unsigned short));
    return ret;
}
...
float*          M3GReader:: getFloat32Array (int size)
{
    ...
    stream->read ((char*)ret, size*sizeof(float));
    return ret;
}
...
const unsigned char* InMemoryStream:: pointer (int offset) const
{
